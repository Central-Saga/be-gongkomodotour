...
class TripDurationRepository implements TripDurationRepositoryInterface
{
    ...
    {
        try {
            return $this->tripduration->where('trip_id', $trip_id)->whereNotIn('id', $existing_id)->delete();
        } catch (\Exception $e) {
    ...
    {
        try {
            $deleted = $this->tripduration->where('trip_id', $trip_id)->delete();
            Log::info("Deleted {$deleted} trip durations for trip_id {$trip_id}");
            return true;
        } catch (\Exception $e) {
            Log::error("Failed to delete trip durations with trip_id {$trip_id}: {$e->getMessage()}");
            return false;
        }
    }
